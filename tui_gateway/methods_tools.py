...
def _(rid, params: dict) -> dict:
    ...
    if name == "goal":
        ...
        # the ``--file <path>`` form: resolve it relative to this session's
        # cwd before mutating goal state. File contents are data, not another
        # slash-command dispatch, because subcommands were classified above
        # from the original argument.
        from hermes_cli.goals import GoalInputError, resolve_goal_input

        try:
            goal_text, contract = resolve_goal_input(
                arg,
