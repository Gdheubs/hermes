...
def _(rid, params: dict) -> dict:
    ...
    # Quick aliases must be projected back to the TUI instead of executing in
    # the isolated slash worker.  The frontend can then route aliases targeting
    # native commands such as ``/image``, preserving both the attached-image
    # token and any trailing prompt text.
    quick_command = _load_cfg().get("quick_commands", {}).get(_cmd_base)
    if isinstance(quick_command, dict) and quick_command.get("type") == "alias":
