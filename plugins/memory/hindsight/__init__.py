        ...
        def _run():
            ...
            try:
                if self._prefetch_method == "reflect":
                    ...
                    if self._auto_retain_filter_enabled:
                        text = self._sanitize_filtered_text(text)
                        if self._recall_text_is_noise(text):
                            text = ""
                else:
                    recall_kwargs: dict = {
                    ...
                    text = "\n".join(
                        f"- {result_text}"
                        for result_text in self._filtered_recall_result_texts(resp.results)
                    )
                # Recall results were already sanitized/vetoed individually.
                # Do not apply recall_skip_patterns to the joined aggregate:
                # one noisy result (or a cross-result regex) must not suppress
