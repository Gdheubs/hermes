...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Fast-forward only. A diverged feature branch can show behind>0
            # with a tiny tip count, or behind==0 when neither tip is an
            # ancestor of the other (#68484). Run the ancestry check for every
            # successful full-clone count so a diverged branch is never
            # reported as a small fast-forward (or as up-to-date).
