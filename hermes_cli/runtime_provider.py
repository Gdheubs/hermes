        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in custom_provider_aliases(
                ...
                if base_url:
                    ...
                    # use the legacy ``api_mode`` spelling. Accept both, but
                    # prefer canonical ``transport`` so runtime routing matches
                    # the Desktop management API. Without this lift, migrated
                    # configs silently downgrade to chat_completions.
                    api_mode = _parse_api_mode(entry.get("transport") or entry.get("api_mode"))
                    if api_mode:
