...
function McpSetupPending({ args }: ToolCallMessagePartProps) {
    ...
    try {
      ...
      if (action === 'authorize') {
        const flow = await completeMcpDesktopOAuth({
          serverName: server,
          start: name => authMcpServer(name, undefined, sourceScope),
          status: flowId => getMcpOAuthFlow(flowId, undefined, sourceScope),
          cancelled: () => cancelRef.current,
          cancel: flowId => cancelMcpOAuthFlow(flowId, undefined, sourceScope),
      ...
      if (resolved === undefined) {
        const catalog = await getMcpCatalog(undefined, sourceScope)
      ...
      const res = await installMcpCatalogEntry(server, envDraft, undefined, sourceScope)
      ...
      if (res.background && res.action) {
        for (;;) {
          const status = throwIfCancelled(await getActionStatus(res.action, 1, undefined, sourceScope))
  ...
  }, [action, copy, entry, envDraft, gateway, request, respond, server, sourceScope])
