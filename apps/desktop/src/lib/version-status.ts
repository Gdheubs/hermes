...
export function resolveVersionStatus({
  ...
  // behind === -2 is UPDATE_DIVERGED (#68484): histories forked, not a
  // fast-forward count. Do not render it as (+-2).
  ...
  // `(update)` is the fallback for a backend that knows it's stale but can't
  // count (pip, non-git checkout).
  const hint = busy
    ? ''
    : behind > 0
      ? ` (+${behind})`
