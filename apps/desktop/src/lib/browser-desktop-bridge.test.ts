...
describe('browser-hosted Desktop bridge', () => {
  ...
  it('maps the Desktop terminal rail onto the authenticated browser PTY socket', async () => {
    const win = mutableWindow()
    win.__HERMES_SESSION_TOKEN__ = 'served-token'
    win.__HERMES_BASE_PATH__ = '/hermes'
    ...
    const fetchMock = vi.fn(async (input: URL | RequestInfo) => {
      ...
      if (url.pathname.endsWith('/api/terminal/cwd')) {
        return new Response(JSON.stringify({ cwd: '/work/live' }), { status: 200 })
      }
        ...
        queueMicrotask(() => {
          ...
          this.onmessage?.(
            new MessageEvent('message', {
              data: JSON.stringify({ type: 'ready', id: 'server-id', cwd: '/work', shell: 'bash' })
            })
          )
        })
      }
    ...
    expect(session).toMatchObject({ cwd: '/work', shell: 'bash' })
    expect(url.pathname).toBe('/hermes/api/terminal')
    ...
    socket.emitBytes('hello from termux')
    await Promise.resolve()
    expect(output).toHaveBeenCalledWith('hello from termux')
    ...
    await expect(win.hermesDesktop!.terminal.write(session.id, 'pwd\r')).resolves.toBe(true)
    await expect(win.hermesDesktop!.terminal.resize(session.id, { cols: 80, rows: 24 })).resolves.toBe(true)
    expect(socket.sent).toEqual(['pwd\r', '\u001b[RESIZE:80;24]'])
    await expect(win.hermesDesktop!.terminal.cwd(session.id)).resolves.toBe('/work/live')
  ...
  it('buffers an early shell prompt until Desktop registers its terminal data listener', async () => {
    const win = mutableWindow()
    win.__HERMES_SESSION_TOKEN__ = 'served-token'
    ...
    class PromptWebSocket {
      ...
      constructor(_url: string | URL) {
        queueMicrotask(() => {
          ...
          this.onmessage?.(
            new MessageEvent('message', { data: JSON.stringify({ type: 'ready', cwd: '/home', shell: 'bash' }) })
          )
