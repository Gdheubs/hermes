...
import { SCHEMA_ZH_DESCRIPTION, SCHEMA_ZH_LABEL, SCHEMA_ZH_SEGMENT, SCHEMA_ZH_TERM } from './schemaZh'
...
export const zh: TranslationOverlay = {
  ...
  chatSidebar: {
    ...
    reasoningEffortSet: '推理强度已设为 {effort}。运行 `/new` 或刷新页面后，本次对话才会使用新设置。',
    modelSetRequiresReload: '模型已设为 {model}。运行 `/new` 或刷新页面后，本次对话才会使用新模型。',
    ...
    sessionTokenUnavailable: '会话令牌不可用。请通过 `hermes dashboard` 打开此页面，不要直接访问。',
  ...
  status: {
    ...
    recentSessions: '最近会话',
    restartGateway: '重启网关',
    restartGatewayConfirmMessage: '这将重启 Hermes 网关进程。已连接的渠道和活动会话会在重启后重新连接。',
    ...
    updateHermesConfirmMessage: '将运行 hermes update，完成后重启网关。更新完成前，活动会话会保留提示词缓存。',
  ...
  sessions: {
    ...
    sourceLocal: '本地',
    sourceApiServer: 'API 服务器',
    sourceCron: '定时任务',
    sourceTool: '工具',
    sourceHermesFlow: 'Hermes Flow',
    sourceVulcanDelegate: 'Vulcan 委派',
    roles: {
      user: '用户',
      assistant: '助手',
      system: '系统',
  ...
  modelSettings: {
    ...
    moaRecursiveError: 'MoA 预设不能引用或聚合 Mixture of Agents 服务商，因为不支持递归 MoA。',
    ...
    remove: '移除',
    resetAll: '全部重置',
    resetAuxDescription: '要将所有辅助任务重置为自动选择吗？这会替换已经设置的逐任务覆盖。',
  ...
  channels: {
    ...
    configuredSummary: '已配置 {configured}/{total} 个渠道。凭据将写入 {path}；网关下次重启时会连接所有已启用的渠道。',
    enablePlatform: '启用 {name}',
    errorToast: '错误：{error}',
    failedToRestart: '重启失败：{error}',
    failedToSave: '保存失败：{error}',
    fixHighlightedFields: '请先修正标出的字段，再保存。',
    invalidTelegramBotToken: '请粘贴 @BotFather 提供的完整令牌（例如 123456789:ABC…）。',
    invalidTelegramUserId: '{value} 不是有效的 Telegram 数字用户 ID。',
    invalidSlackTokenPrefix: '{field} 必须以 {prefix} 开头',
    invalidSlackMemberId: '{value} 不是有效的 Slack 成员 ID。请使用类似 U01ABC2DEF3 的 ID。',
    ...
    telegramManualIntro: '连接你已有的机器人，或先在 Telegram 中创建机器人，再填写此表单。',
    telegramManualCreateBot: '打开 @BotFather，发送 /newbot，并按照提示完成创建。',
    telegramManualCopyToken: '复制 BotFather 提供的完整机器人令牌。',
    telegramManualFindUserId: '向 @userinfobot 发送消息以获取你的 Telegram 数字用户 ID，然后在下方添加，即可立即访问。',
    telegramOpenBotFather: '打开 @BotFather',
    telegramFindUserId: '查找我的用户 ID',
    telegramPairingFallback: '允许的用户可以留空。之后新的私信用户会收到配对码，你可以在「配对」页面批准。',
    test: '测试',
    onboarding: {
      add: '添加',
      ...
      telegramChooseMethodHint: '两种方式都会连接由你控制的机器人，并且只把凭据保存到当前 Hermes 安装中。',
      telegramQuickSetup: '快速设置',
      telegramQuickSetupHint: '扫描二维码并在 Telegram 中确认。Hermes 会创建机器人并自动识别你的 Telegram 用户 ID。',
      telegramCreateWithQr: '通过二维码创建',
      telegramOwnBot: '使用你自己的机器人',
      telegramOwnBotHint: '使用 @BotFather 创建机器人，或输入现有机器人的令牌，并选择允许使用它的用户。',
      telegramManualSetup: '手动设置',
      telegramConfiguredReplaceHint: '已配置 Telegram 凭据。保存新的二维码设置或机器人令牌时，会替换当前机器人。',
      ...
      whatsappBridgePreparing: '正在准备 WhatsApp 桥接服务。就绪后会在此显示二维码。',
      whatsappBridgeStarting: '正在启动 WhatsApp 配对桥接服务。就绪后会在此显示二维码。',
      ...
      whatsappPairingFallback: '如果未输入允许的号码，Hermes 会回复配对码。请在管理面板的「配对」页批准。',
      whatsappQrExpired: 'WhatsApp 二维码设置已过期。请重新启动二维码设置。',
      whatsappQrInstructions: '在手机上打开 WhatsApp，进入「已关联设备」并扫描。该二维码不是浏览器链接。',
      whatsappQrPending: '正在等待 WhatsApp 提供二维码…',
      whatsappQrScanHint: '请使用 WhatsApp 的「已关联设备」扫描，不要使用相机应用。',
      whatsappSaveFirst: '保存并重启网关。',
      whatsappSelfChatAutoAllow: '保存时，「给自己发消息」模式会自动允许已关联账号。',
      whatsappSelfChatMessage: '重启后，在已关联账号中打开「给自己发消息」，然后向 Hermes 发送消息。',
      whatsappSetupFailed: 'WhatsApp 设置失败。',
      whatsappStandardMessage: '重启后，使用另一个 WhatsApp 账号向已关联账号发起聊天，并向 Hermes 发送消息。',
      whatsappUnknownDmHint: '保存后，未知私信号码若不在允许列表中，将使用 Hermes 配对码。',
      whatsappLinkedNeedsRestart: 'WhatsApp 已关联，但 Hermes 尚未开始监听。请保存并重启网关以完成设置。'
    },
    state: {
      connected: '已连接',
  ...
  cron: {
    ...
    delivery: {
      ...
      noneConfigured: '尚未配置消息平台。请先在「渠道」页面完成设置，再投递报告。'
    }
  },
  ...
  profiles: {
    ...
    confirmDeleteMessage: '将永久删除配置档案「{name}」，包括配置、密钥、记忆、会话、技能和定时任务。此操作无法撤销。',
    created: '已创建',
    deleted: '已删除',
    renamed: '已重命名',
    ...
    modelSaved: '模型已更新',
    modelSelect: '选择模型',
    modelSaveAfterCreateFailed: '多 Agent 配置已创建，但模型设置未能保存；请在配置编辑器中重新设置。',
    build: '引导创建',
    actions: '操作',
    manageSkills: '管理技能与工具',
    activeSetHint: '管理面板已切换为管理 {name}。后续启动的 CLI 和网关也会使用此配置档案。',
    activeSetToast: '已将 {name} 设为活动配置档案；管理面板及后续启动的 CLI 和网关都会使用它。'
  },
  ...
  pluginsPage: {
    ...
    descriptions: {
      ...
      kanban: '多 Agent 协作看板——跨列移动卡片、阅读评论线程，并查看每项任务由哪个配置档案运行。'
    },
    inactive: '未启用',
    installBtn: '安装',
    ...
    rescanHint: '在磁盘上新增文件后重新扫描，使管理面板侧边栏载入新的清单文件（manifest）。',
  ...
  skills: {
    ...
    managingProfile: '正在管理配置档案「{name}」；此处的开关只对该档案生效，不影响当前管理面板。',
    browseHub: '浏览技能中心',
    learnSkill: '学习技能',
    newSkill: '新建技能',
    configure: '配置',
    ...
    url: 'URL',
    urlPlaceholder: 'https://docs.example.com/api（通过 web_extract 获取）',
  ...
  oauth: {
    ...
    notConnectedHint: '尚未连接。可用时请点击「登录」，也可以在终端运行下方命令。',
    runInTerminal: '在终端中。',
    ...
    disconnectDescription: '这将移除 {provider} 已存储的 OAuth 访问令牌。需要重新授权后才能继续使用。'
  },
  ...
  mcp: {
    ...
    bearerTokenStorageHint: '令牌存储在此配置档案的 .env 中；config.yaml 只保留环境变量引用。',
    oauthAddHint: '添加服务器后，请使用“认证”继续操作。Hermes 会在运行管理面板后端的主机上打开 OAuth 浏览器。',
    oauthComplete: '{name}：OAuth 认证已完成',
    oauthFailed: '{name}：{error}',
    oauthError: 'OAuth 错误：{error}',
    oauthPopupBlocked: 'OAuth 弹窗被浏览器拦截，请允许此面板打开弹窗后重试',
  ...
  profileBuilder: {
    ...
    oauthAfterCreateHint: '请先创建多 Agent 配置，然后在 MCP 页面完成 OAuth 授权。',
    addServer: '添加服务器',
    remove: '移除',
    name: '名称',
    description: '说明',
  ...
  systemPage: {
    ...
    confirm: {
      ...
      updateDescription: '将运行“hermes update”（{command}），并在更新完成后重启网关。',
      updateNow: '立即更新',
      ...
      restoreDescription: '此操作会使用 {archive} 中的内容覆盖当前 Hermes 配置、技能、会话和数据，且无法撤销。',
      restore: '恢复'
    },
    hooks: {
      ...
      securityWarning: 'Shell 钩子可在此主机上运行任意命令。请仅添加你信任的脚本。配置会在下次网关或会话重启后生效。',
      creating: '正在创建…',
      create: '创建钩子',
      title: 'Shell 钩子',
      new: '新建钩子',
