...
def test_queue_prefetch_does_not_apply_aggregate_veto_to_siblings():
    p = _provider()
    p._recall_skip_patterns = [r"(?is)temporary debug.*useful preference"]
    ...
    assert p._prefetch_result == (
        "- temporary debug lifecycle artifact\n"
        "- Useful preference after."
    )
