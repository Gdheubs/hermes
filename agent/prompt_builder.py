...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    try:
        ...
        from tools.terminal_tool import _get_or_create_environment  # type: ignore
    ...
    try:
        env, _actual_env_type, _created = _get_or_create_environment("default")
...
def build_environment_hints() -> str:
    ...
    if backend != "local" and not is_remote_backend:
        try:
            from tools.terminal_tool import _backend_uses_container_paths

            is_remote_backend = _backend_uses_container_paths(backend)
        except Exception:
            logger.debug(
                "Could not classify terminal backend %s for prompt hints",
                backend,
                exc_info=True,
            )
