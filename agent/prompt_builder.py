...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (material-use policy)\n"
            "Before replying, scan the skills below. Load a skill with skill_view(name) when "
            "its instructions are likely to materially change how you execute or verify the "
            "task: complex workflows, code changes/reviews, planning, debugging, external "
            "integrations, user-stated preferences, product/UX strategy, or other high-risk "
            "work. You may proceed without loading a skill for simple factual answers, "
            "low-risk copy/wording, or tasks where no listed skill would change the result.\n"
            "If the user names a skill or asks to use one, load it. When you load a skill, "
            "follow its instructions and linked files as needed. Skills contain specialized "
            "knowledge — API endpoints, tool-specific commands, proven workflows, and the "
            "user's preferred quality standards.\n"
            "Mandatory cases: whenever the user asks you to configure, set up, install, "
            "enable, disable, modify, or troubleshoot Hermes Agent itself — its CLI, config, "
            "models, providers, tools, skills, voice, gateway, plugins, or any feature — "
            "load the `hermes-agent` skill first. For code changes, tests, PR/review, "
            "production, billing, or customer-data work, live product QA, or actions with "
            "external side effects, load the matching skill(s) before acting (for example: "
            "verification, TDD, GitHub, or product-context skills).\n"
            "If a relevant skill is disabled or unavailable, state that and either re-enable "
            "it/ask when needed or continue with available tools if safe. If a skill has "
            ...
            "</available_skills>\n"
            "\n"
            "Only proceed without loading a skill when no available skill would materially "
            "change execution or verification."
            + hidden_note
        )
