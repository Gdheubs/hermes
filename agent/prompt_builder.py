...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Before replying, scan the skills below and load the ones with DIRECT "
            "relevance to the task in front of you — a skill whose subject IS the "
            "task, not one that is merely adjacent or tangentially related. Load a "
            "relevant skill with skill_view(name) and follow its instructions; "
            "skills encode specialized knowledge (API endpoints, tool-specific "
            "commands, proven workflows) and the user's preferred conventions and "
            "quality standards, so a directly-relevant skill beats a general-purpose "
            "approach even for tasks you already know how to do.\n"
            "Cap automatic pre-task loading at 2 skill bodies total, including "
            "any mandatory load below. Honor the explicit mandatory rules below "
            "before choosing any remaining slot. When more skills look like "
            "candidates than the remaining slots allow, load the single most "
            "specific umbrella/domain skill and DEFER the additional bodies until a "
            "concrete need for them actually appears mid-task — do not front-load "
            "every plausible skill. Loading fewer, more-targeted skills keeps "
            "context focused and avoids redundant skill_view calls. This cap is a "
            "default, not a hard ceiling: an explicit user request to load specific "
            "skills, or a hard production or security review gate that genuinely "
            "requires them, may exceed 2 when the task truly demands it.\n"
            ...
            "</available_skills>\n"
            "\n"
            "Proceed without loading a skill when none is directly relevant to the task."
            + hidden_note
        )
