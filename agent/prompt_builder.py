...
KANBAN_GUIDANCE = (
    ...
    "## Lifecycle\n"
    "\n"
    "1. **Orient.** The dispatcher includes an authoritative `# Kanban task …` "
    "context block in your first user message. Read that block before using "
    "tools. Call `kanban_show()` only if the block is absent or you need to "
    "refresh task state after the run started; it defaults to your task and "
    "returns title, body, parent handoffs, prior attempts, comments, and recent "
    "events.\n"
